...
export const STOCK_SIGNAL_PROFILE_SCHEMA_VERSION = 'stock-signal-profiles-v5-direction-timing-split';
...
const PROFILE_DEFINITIONS = Object.freeze({
  responsive: Object.freeze({
    id: 'responsive', version: 'responsive-v3.0.0-direction-timing-split', label: '敏捷观察', role: 'observe',
    actionCapable: true,
    parameters: Object.freeze({
      ...
      strategy: Object.freeze({ fastMa: 20, slowMa: 50 }),
      minimumBars: 60,
  ...
  balanced: Object.freeze({
    ...
    id: 'balanced', version: 'balanced-v4.0.0-direction-timing-split', label: '均衡决策', role: 'formal',
    ...
    parameters: Object.freeze({
      ...
      trend: Object.freeze({ fastMa: 20, slowMa: 50, mode: 'structure_and_slope' }),
  ...
  confirmed: Object.freeze({
    id: 'confirmed', version: 'confirmed-v3.0.0-direction-timing-split', label: '稳健确认', role: 'confirm',
    actionCapable: true,
    parameters: Object.freeze({
      ...
      strategy: Object.freeze({ fastMa: 50, slowMa: 200 }),
      minimumBars: 200,
...
function volumePriceCorrelation(closes, volumes, window = 20) {
  if (!Array.isArray(closes) || closes.length < window + 1) return null;
  const returns = [];
  const alignedVolumes = [];
  for (let index = closes.length - window; index < closes.length; index += 1) {
    const previous = finite(closes[index - 1]);
    const current = finite(closes[index]);
    const volume = finite(volumes[index]);
    if (!(previous > 0) || current == null || !(volume > 0)) continue;
    returns.push(current / previous - 1);
    alignedVolumes.push(volume);
  }
  if (returns.length < 10) return null;
  const meanReturn = returns.reduce((sum, value) => sum + value, 0) / returns.length;
  const meanVolume = alignedVolumes.reduce((sum, value) => sum + value, 0) / alignedVolumes.length;
  let numerator = 0;
  let returnVariance = 0;
  let volumeVariance = 0;
  for (let index = 0; index < returns.length; index += 1) {
    const returnDelta = returns[index] - meanReturn;
    const volumeDelta = alignedVolumes[index] - meanVolume;
    numerator += returnDelta * volumeDelta;
    returnVariance += returnDelta ** 2;
    volumeVariance += volumeDelta ** 2;
  }
  const denominator = Math.sqrt(returnVariance * volumeVariance);
...
function directionForSignal(signal) {
  ...
  return value.includes('BULLISH') || value.includes('BUY') ? 1
    : value.includes('BEARISH') || value.includes('SELL') ? -1 : 0;
}
...
function profileStatus(profile, signal, confirmed) {
  ...
  if (profile.role === 'formal') return `FORMAL_${side}`;
  return confirmed ? `CONFIRMED_${side}` : `PENDING_${side}`;
}

function profileVote({ key, vote, weight, text, role = 'direction' }) {
  return { key, vote: +vote.toFixed(4), weight, text, role };
}
...
function movingAverageSlopePct(closes, period, lookback = 5) {
  if (!Array.isArray(closes) || closes.length < period + lookback) return null;
  const current = smaArr(closes, period);
  const previous = smaArr(closes.slice(0, -lookback), period);
  return current != null && previous > 0 ? (current / previous - 1) * 100 : null;
}

export function balancedVolumePriceVote(correlation, roc) {
  const corr = finite(correlation);
  const momentum = finite(roc);
  if (corr == null) return 0;
  if (corr > 0.3) return momentum != null && momentum > 0 ? 0.6 : -0.6;
  if (corr < -0.3) return momentum != null && momentum <= 0 ? -0.4 : -0.2;
  return 0;
}

function balancedRelativeVote(relativeFast, relativeSlow, regimeState) {
  let vote = 0;
  if (relativeFast != null) {
    if (relativeFast >= 5 && (relativeSlow == null || relativeSlow >= 0)) vote = 0.7;
    else if (relativeFast >= 2) vote = 0.4;
    else if (relativeFast <= -5 && (relativeSlow == null || relativeSlow <= 0)) vote = -0.7;
    else if (relativeFast <= -2) vote = -0.4;
  }
  return regimeState === 'range' ? vote : clamp(vote * 1.2, -1, 1);
}

// All three personalities expose the same six pieces of evidence, but only
// trend, MACD, volume confirmation and relative strength determine direction.
// RSI and Bollinger position describe timing; an overbought reading must never
// turn an intact uptrend into a bearish direction score.
function computeProfile(profile, context) {
  ...
  const marketRegime = context?.sharedMarketContext?.marketRegime?.key || 'range';
  const balanced = profile.id === FORMAL_SIGNAL_PROFILE_ID;
  const bands = balanced
    ? balancedRsiBandsForRegime(marketRegime)
    : rsiBands(closes, parameters.rsi);
  const rsi = rsiWilderAt(closes, closes.length - 1, parameters.rsi.period);
  const macd = macdSnapshot(closes, parameters.macd);
  const trendFastPeriod = parameters.trend.fastMa;
  const trendSlowPeriod = parameters.trend.slowMa;
  const trendFast = smaArr(closes, trendFastPeriod);
  const trendSlow = smaArr(closes, trendSlowPeriod);
  const trendFastSlopePct = movingAverageSlopePct(closes, trendFastPeriod);
  const strategyFast = smaArr(closes, parameters.strategy.fastMa);
  const strategySlow = smaArr(closes, parameters.strategy.slowMa);
  ...
  const volPriceCorr = balanced ? volumePriceCorrelation(closes, volumes, parameters.volume.lookback) : null;
  ...
  if (rsi != null && bands && bands.hardHigh - bands.hardLow >= 1e-6) {
    if (balanced ? rsi < bands.hardLow : rsi <= bands.hardLow) rsiVote = 1;
    else if (balanced ? rsi < bands.softLow : rsi <= bands.softLow) rsiVote = 0.5;
    else if (balanced ? rsi > bands.hardHigh : rsi >= bands.hardHigh) rsiVote = -1;
    else if (balanced ? rsi > bands.softHigh : rsi >= bands.softHigh) rsiVote = -0.5;
  }
  votes.push(profileVote({ key: 'rsi', vote: rsiVote, weight: PROFILE_VOTE_WEIGHTS.rsi, role:'timing',
    text: balanced ? `RSI${parameters.rsi.period} market-regime` : `RSI${parameters.rsi.period} rolling-quantile` }));

  let macdVote = 0;
  if (macd.histogram != null) {
    const growing = macd.previous == null || macd.histogram >= macd.previous;
    if (macd.histogram > 0) macdVote = balanced
      ? (bands?.state === 'bear' || growing ? 1 : 0.5)
      : (growing ? 1 : 0.5);
    else if (macd.histogram < 0) macdVote = balanced
      ? (bands?.state === 'bull' || !growing ? -1 : -0.5)
      : (!growing ? -1 : -0.5);
  }
  ...
  if (trendFast != null && trendSlow != null) {
    ...
    else if (current > trendFast && trendFastSlopePct != null && trendFastSlopePct > 0) trendVote = 0.45;
    else if (current < trendFast && trendFastSlopePct != null && trendFastSlopePct < 0) trendVote = -0.45;
  }
  votes.push(profileVote({ key: 'trend', vote: trendVote, weight: PROFILE_VOTE_WEIGHTS.trend,
    text: `price/MA${trendFastPeriod}/MA${trendSlowPeriod} + slope` }));

  let bollVote = 0;
  if (boll?.pctB != null) {
    if (balanced && bands?.state === 'bull') bollVote = boll.pctB < 0.15 ? 0.8 : boll.pctB > 0.95 ? -0.4 : 0;
    else if (balanced && bands?.state === 'bear') bollVote = boll.pctB < 0.05 ? 0.3 : boll.pctB > 0.85 ? -0.8 : 0;
    else if (balanced) bollVote = boll.pctB < 0.20 ? 0.8 : boll.pctB > 0.80 ? -0.8 : 0;
    else if (boll.pctB <= 0.20) bollVote = 0.5;
    else if (boll.pctB >= 0.80) bollVote = -0.5;
  }
  votes.push(profileVote({ key: 'volatility', vote: bollVote, weight: PROFILE_VOTE_WEIGHTS.volatility, role:'timing', text: `BOLL ${parameters.bollinger.period}/${parameters.bollinger.multiplier}` }));

  let volumeVote = balanced
    ? balancedVolumePriceVote(volPriceCorr, roc)
    : 0;
  if (!balanced && volRatio != null && volRatio >= parameters.volume.ratio && roc != null) volumeVote = roc > 0 ? 0.6 : roc < 0 ? -0.6 : 0;
  votes.push(profileVote({ key: 'volume', vote: volumeVote, weight: PROFILE_VOTE_WEIGHTS.volume,
    text: balanced ? 'signed return-volume correlation' : `RVOL${parameters.volume.lookback}` }));

  let relativeVote = balanced
    ? balancedRelativeVote(relativeFast, relativeSlow, bands?.state || 'range')
    : 0;
  if (!balanced && relativeFast != null) {
  ...
  const directionVotes = votes.filter(vote => vote.role === 'direction');
  const score = aggregateVotes(directionVotes);
  const signal = signalForProfileScore(score);
  const direction = directionForSignal(signal);
  const directionalComponents = directionVotes.filter(vote => direction && vote.vote * direction > 0.1).length;
  ...
  const confirmed = profile.role === 'formal' ? direction !== 0
    : requiresConfirmation && direction !== 0
  ...
  return {
    ...
    metrics: {
      ...
      trendFast: finite(trendFast), trendSlow: finite(trendSlow), trendFastSlopePct: finite(trendFastSlopePct),
      strategyFast: finite(strategyFast), strategySlow: finite(strategySlow),
      bollMiddle: finite(boll?.middle), bollUpper: finite(boll?.upper), bollLower: finite(boll?.lower), bollPctB: finite(boll?.pctB),
      volumeRatio: finite(volRatio), volumePriceCorrelation: finite(volPriceCorr),
      relativeFast, relativeSlow, roc, oneDayReturnPct:currentRoc(closes, 1), marketRegime,
    },
    votes, directionVotes,
  };
}

export function computeSignalProfileBundle({ closes, volumes, relativeStrength, sharedMarketContext, requestedProfileId = FORMAL_SIGNAL_PROFILE_ID } = {}) {
  const profiles = {
    responsive: computeProfile(PROFILE_DEFINITIONS.responsive, { closes, volumes, relativeStrength, sharedMarketContext }),
    balanced: computeProfile(PROFILE_DEFINITIONS.balanced, { closes, volumes, relativeStrength, sharedMarketContext }),
    confirmed: computeProfile(PROFILE_DEFINITIONS.confirmed, { closes, volumes, relativeStrength, sharedMarketContext }),
  };
  for (const profile of Object.values(profiles)) {
    profile.strategy = buildStockProfileStrategy(profile, sharedMarketContext || {});
  }
