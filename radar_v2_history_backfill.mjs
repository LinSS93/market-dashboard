...
import { adapterFor } from './radar_v2_market.mjs';
...
import { scoreCandidate, fetchEventFactsAsOf } from './radar_v2_scoring.mjs';
import { linkObservationsForRun } from './radar_v2_dossier_producer.mjs';
import { backfillOutcome } from './radar_v2_outcomes.mjs';
...
export function backfillHistoricalMarketDay({ market, tradeDate, outcomeLimit = 50, force = false } = {}) {
  ...
  if (existing && force) {
    ...
    getRadarV2Db().prepare('DELETE FROM radar_v2_runs WHERE id = ?').run(existing.id);
  }
