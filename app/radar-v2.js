// radar-v2 页面逻辑（信息架构重构：今日研究队列 + 档案库）
//
// 消费 API：
//   /radar_v2/symbols           — 跨通道 distinct symbol 列表（今日研究队列 + 档案库）
//   /radar_v2/symbol-dossiers   — 某 symbol 全部 dossier 按 channel 分组（详情）
//   /radar_v2/dossier-detail    — 单个 dossier 详情（含 confirmation/invalidation + 评估审计）
//   /radar_v2/kline             — V2-owned 日K线
//   /radar_v2/company-profile   — V2-owned 公司简介
...
(function () {
  ...
  async function fetchSymbols(market, { limit = 100, offset = 0, search = '' } = {}) {
    ...
    const resp = await fetch(`/radar_v2/symbols?${params}`, { cache: 'no-store' });
  ...
  async function dismissSymbolFromQueue(market, symbol) {
    try {
      const resp = await fetch(
        `/radar_v2/queue/dismiss?market=${encodeURIComponent(market)}&symbol=${encodeURIComponent(symbol)}`,
        { method: 'POST' }
      );
      if (!resp.ok) throw new Error(`HTTP ${resp.status}`);
      const json = await resp.json();
  ...
  async function restoreSymbolToQueue(market, symbol) {
    try {
      const resp = await fetch(
        `/radar_v2/queue/restore?market=${encodeURIComponent(market)}&symbol=${encodeURIComponent(symbol)}`,
        { method: 'POST' }
      );
      if (!resp.ok) throw new Error(`HTTP ${resp.status}`);
      const json = await resp.json();
  ...
  async function fetchDismissed(market) {
    ...
    if (market) params.set('market', market);
    const resp = await fetch(`/radar_v2/queue/dismissed?${params}`, { cache: 'no-store' });
    if (!resp.ok) throw new Error(`HTTP ${resp.status}`);
    const json = await resp.json();
    if (!json.ok) throw new Error(json.error || '查询失败');
    return json.data || [];
  ...
  async function fetchCompanyProfile(market, symbol) {
    const resp = await fetch(`/radar_v2/company-profile?market=${encodeURIComponent(market)}&symbol=${encodeURIComponent(symbol)}`, { cache: 'no-store' });
  ...
  async function fetchKline(market, symbol, days = 120) {
    const resp = await fetch(`/radar_v2/kline?market=${encodeURIComponent(market)}&symbol=${encodeURIComponent(symbol)}&days=${days}`, { cache: 'no-store' });
    if (!resp.ok) return null;
    const json = await resp.json();
  ...
  async function fetchFinancial(market, symbol) {
    const resp = await fetch(`/radar_v2/financial?market=${encodeURIComponent(market)}&symbol=${encodeURIComponent(symbol)}`, { cache: 'no-store' });
    if (!resp.ok) return null;
    const json = await resp.json();
    return json.ok ? json.data : null;
  }
  ...
  async function fetchSymbolDossiers(market, symbol) {
    ...
    const resp = await fetch(`/radar_v2/symbol-dossiers?${params}`, { cache: 'no-store' });
    if (!resp.ok) throw new Error(`HTTP ${resp.status}`);
    const json = await resp.json();
    if (!json.ok) throw new Error(json.error || '查询失败');
    return json.data;
  ...
  async function fetchDossierDetail(id) {
    const resp = await fetch(`/radar_v2/dossier-detail?id=${id}`, { cache: 'no-store' });
    if (!resp.ok) throw new Error(`HTTP ${resp.status}`);
    const json = await resp.json();
    if (!json.ok) throw new Error(json.error || '查询失败');
    return json.data;
  ...
  // tier 阈值与 radar_v2_scoring.mjs 的 scoreCandidate 一致（>=70 high / >=50 medium / <50 low）。
  ...
  // 点击后调用 POST /radar_v2/asset-audit，写入审计表，下次刷新队列时生效
  ...
  function bindAssetAudit(token) {
    ...
    for (const btn of buttons) {
      btn.addEventListener('click', async () => {
        ...
        btn.textContent = '提交中…';
        try {
          const resp = await fetch('/radar_v2/asset-audit', {
            method: 'POST',
  ...
  function switchTab(tab) {
    ...
    document.querySelectorAll('.radar-v2-tab').forEach((btn) => {
