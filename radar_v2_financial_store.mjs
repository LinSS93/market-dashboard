...
import { getRadarV2Db, lazyStmt } from './radar_v2_schema.mjs';
import { deriveFinancialAvailability, isFinancialTimingUsable } from './radar_v2_financial_timing.mjs';
import { batchMatchHkAnnouncements, ensureHkexAnnouncementCache } from './radar_v2_hkex_announcement_matcher.mjs';
...
export function ensureRadarV2FinancialStore(db = getRadarV2Db()) {
...
export async function importRetiredFinancialArchive({ market = null, limit = 5000, now = Date.now(), skipHkexBackfill = false } = {}) {
  const db = ensureRadarV2FinancialStore();
