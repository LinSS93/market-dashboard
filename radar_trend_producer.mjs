    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (i > 0 && i % RENEW_INTERVAL === 0) {
        ...
        if (renewResult.changes === 0) {
          ...
          console.log(`[radar_trend] job ${jobId} lease 被抢占，停止 batch（已处理 ${i}/${items.length}）`);
          break;
        }
      }
      ...
      try {
        ...
        stats.failed += 1;
        console.log(`[radar_trend] ${job.market}:${item.symbol} 处理失败: ${e?.message || e}`);
      }
    }
 ...
 *
 * P1-1: 若 RADAR_TREND_ENABLED 关闭，直接跳过（kill switch）。
...
export function isTrendEnabledForMarket(market) {
  const enabled = process.env.RADAR_TREND_ENABLED;
...
/**
 * Shadow 入口（异步）：scanner run 完成后调用，受 RADAR_TREND_ENABLED 控制。
 *
...
export function backfillMissingTrendJobs({ limit = 50 } = {}) {
  ...
  if (recovered > 0) {
    console.log(`[radar_trend] 回补遗漏 source run: ${recovered} 个`);
  }
 ...
 *
 * P1-1: kill switch——若 RADAR_TREND_ENABLED 关闭或无启用市场，直接返回，不写库。
...
export function fullTrendReconcile(opts = {}) {
  ...
  if (report.permanent_failures.length > 0) {
    console.log(`[radar_trend] 永久失败样本 (${report.permanent_failures.length} 条):`);
  ...
  console.log(`[radar_trend] Shadow 报告 [${enabledMarkets.join(',')}]: jobs=${s.total_jobs} incomplete=${s.incomplete_jobs} ` +
