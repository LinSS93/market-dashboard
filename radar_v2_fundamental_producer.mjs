...
import {
  ensureRadarV2FinancialStore,
  getV2FinancialFactsNeedingDossier,
  getV2FinancialHistory,
  importRetiredFinancialArchive,
} from './radar_v2_financial_store.mjs';
import { isFinancialTimingUsable } from './radar_v2_financial_timing.mjs';
import { buildFundamentalDossierEnrichment } from './radar_v2_dossier_enrichment.mjs';
 ...
 *
 * 受 RADAR_V2_FUNDAMENTAL_ENABLED 环境变量控制（默认关闭）：
 *   - 未设置 / 'false' / '0' → 关闭
...
export function isFundamentalEnabledForMarket(market) {
  const enabled = process.env.RADAR_V2_FUNDAMENTAL_ENABLED;
