...
| Tracker / ETF 跟踪 | Review leveraged ETF premium against its underlying: current premium, history percentile, NAV quality, and close-price distribution. / 以溢折价为中心查看杠杆 ETF：当前溢价、历史分位、NAV 质量与收盘溢价分布。 | Verify currency, listing market, and source freshness yourself. / 请自行核验币种、上市市场与数据新鲜度。 |
...
4. **Compare with the stock monitor.** Check current technical/risk context separately. Radar cannot weaken an existing `REDUCE` or `CLOSE` risk condition.
   **再对照股票监控。** 单独查看技术面和风险上下文；机会雷达不能弱化既有的 `REDUCE` 或 `CLOSE` 风险条件。
...
- **Dilemma reversal / 困境反转:** negative signals coexist with positive evidence. Verify which side dominates; it is not a short-sale or rebound instruction. / 负面信号与正向证据并存。需要核验哪一方占主导；这不是做空或抄底指令。
- **High-confidence research / 高置信机会:** more than one research channel confirmed the change with a high score. This increases research priority, not expected return. / 多通道确认且评分较高的变化。这只提升研究优先级，不代表预期收益更高。
- **Pending confirmation / 待确认信号:** a recent fact or state change from a single channel or with a mid-low score. It may still be incomplete, stale, or later invalidated. / 单通道或中低评分的近期事实或状态变化，仍可能不完整、过期或随后被证伪。
- **Audit pending / 待审计:** the instrument classification has not been verified. Confirm that it is an eligible equity before using it for research. / 资产分类尚未审计，暂按普通股处理；用于研究前请先确认它是否为符合条件的股票。
- **Unscored / 无评分:** evidence exists but there is not enough reliable context for a research-order score. / 已有证据，但还不足以给出可靠的研究排序分。
