...
Download `market-dashboard-v0.2.3-windows.zip` from the GitHub Release page, then extract the whole folder to a normal local directory such as `Documents\Market Dashboard`. Do not run it inside the ZIP file.
从 GitHub Release 页面下载 `market-dashboard-v0.2.3-windows.zip`，将整个文件夹解压到普通本地目录，例如 `Documents\Market Dashboard`。不要在 ZIP 文件内部直接运行。
