...
- On the first run it installs the local dependencies. Internet access is needed only for that step.
- It starts the dashboard in a separate terminal window, waits for it to be ready, and opens `http://127.0.0.1:8080`.
- Create the first local administrator in the browser. This account and all later data remain on your computer.

双击 `Start-Market-Dashboard.cmd`。

- 第一次运行会安装本地依赖，仅这一步需要联网。
- 脚本会在单独的终端窗口启动看板、等待它就绪，然后打开 `http://127.0.0.1:8080`。
- 请在浏览器中创建首个本地管理员。该账号和之后的数据都只保存在你的电脑上。
