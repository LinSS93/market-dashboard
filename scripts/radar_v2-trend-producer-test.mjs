...
//
// 运行：node scripts/radar_v2-trend-producer-test.mjs
...
import {
  setRadarV2DbForTest, clearRadarV2DbForTest, getRadarV2Db,
...
} from '../radar_v2_trend_producer.mjs';
...
{
  process.env.RADAR_V2_TREND_ENABLED = 'US';
  ...
  delete process.env.RADAR_V2_TREND_ENABLED;
}
...
console.log('=== D.24 环境变量白名单 ===');
{
  ...
  // 多市场
  process.env.RADAR_V2_TREND_ENABLED = 'US,HK';
...
console.log('=== G.37 旧 dossier（无条件 JSON）标记 legacy_unknown，不补规则/窗口 ===');
{
  ...
  const { TREND_LEGACY_UNKNOWN_VERSION } = await import('../radar_v2_dossier_enrichment.mjs');
...
console.log('=== G.38 半残缺 dossier 标记 legacy，不补 next_review_at ===');
{
  ...
  const { TREND_LEGACY_VERSION } = await import('../radar_v2_dossier_enrichment.mjs');
  markDossierLegacyVersion.run({
    id: halfDossier.id,
