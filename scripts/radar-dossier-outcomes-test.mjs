...
//   7. 批量回填
//
// 运行：node scripts/radar-dossier-outcomes-test.mjs
...
} from '../radar_dossier_outcomes.mjs';
...
console.log('=== 测试 14：旧库迁移回填 absolute_matured ===');
{
  ...
  // 关闭后重新打开，用 setRadarDbForTest 触发 migration
  oldDb.close();
  const migrateDb = new Database(oldDbPath);
  setRadarDbForTest(migrateDb);
