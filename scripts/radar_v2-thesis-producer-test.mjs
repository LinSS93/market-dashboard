...
//
// 运行：node scripts/radar_v2-thesis-producer-test.mjs
...
} from '../radar_v2_thesis_producer.mjs';
...
// === 临时数据库 ===
const tmpDir = mkdtempSync(join(tmpdir(), 'radar-v2-thesis-'));
const tmpDbPath = join(tmpDir, 'test.db');
const db = new Database(tmpDbPath);
db.pragma('journal_mode = WAL');
db.pragma('foreign_keys = ON');

// radar_v2_event_facts 现已由 radar_v2_schema.mjs execSchema 自包含创建。
...
{
  // 默认未设置 RADAR_V2_THESIS_ENABLED
  delete process.env.RADAR_V2_THESIS_ENABLED;
...
process.env.RADAR_V2_THESIS_ENABLED = '1';
...
console.log('=== 测试 17：getThesisStatus ===');
{
  ...
  assert(status.enabled === true, `enabled=true（RADAR_V2_THESIS_ENABLED=1）`);
