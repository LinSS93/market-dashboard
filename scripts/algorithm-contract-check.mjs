...
import { resolveReplayStatus } from '../stock_engine.mjs';
import { balancedVolumePriceVote } from '../stock_signal_profiles.mjs';
...
assert(balancedVolumePriceVote(-0.59, -26.1) < 0, 'negative correlation with falling prices must remain bearish distribution');
assert(balancedVolumePriceVote(-0.45, 8) < 0, 'negative correlation in a rising trend must remain an unconfirmed-volume warning');
assert(balancedVolumePriceVote(0.55, 12) > 0, 'positive correlation with positive momentum remains bullish volume confirmation');
