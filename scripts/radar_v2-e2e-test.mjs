...
//
// 通过 setRadarV2DbForTest 注入临时数据库，调用真实模块，不触碰生产库。
// 预填充 radar_universe_members / radar_daily_bars / radar_v2_bars / radar_v2_event_facts。
//
// 运行：node scripts/radar_v2-e2e-test.mjs
...
import { scheduleRadarV2, stopRadarV2, getSchedulerState, resetSchedulerStateForTest, executeScanForTest, getRoundRobinStartForTest, advanceRoundRobinForTest, isQueueRunningForTest, processDailyQueueForTest, setMarketStatusOverrideForTest, setIsAfterCloseOverrideForTest, inBackoffForTest, hasResumableJobForTest } from '../radar_v2_scheduler.mjs';
import { resetRateLimiterForTest, getRateLimiterState, acquireToken, setNoDelayForTest } from '../radar_v2_rate_limiter.mjs';
...
// 这些表不在 radar_v2_schema 中创建，需要手动建
db.exec(`
...
// 基准指数：US=QQQ, HK=02800, CN=000300（与 radar_v2_market.mjs 保持一致）
...
// === 测试 23: 真实队列生命周期（processDailyQueue + stopRadarV2 联动）===
// P1: 验证 _processDailyQueue 真实派发和 stopRadarV2 的停止行为
// 使用 setMarketStatusOverrideForTest 绕过真实日历，确保测试可重复
console.log('\n[23] 真实队列生命周期（processDailyQueue + stopRadarV2 联动）');
{
  resetThrottleForTest();
  resetSchedulerStateForTest();
  resetRateLimiterForTest();
  ...
  // 3. 验证 stopRadarV2 阻断下一批调度——使用 >1 batch 的任务
  ...
  // P1: 在第一批完成后立即调用 stopRadarV2，阻断后续批次
  let batchCount23b = 0;
  ...
  try {
    // 启动队列，onRunComplete 会在第一批后调用 stopRadarV2
    await processDailyQueueForTest(onRunComplete23b);

    // P1 核心断言：队列已停止
    assert(isQueueRunningForTest() === false, `stopRadarV2 后队列退出（${isQueueRunningForTest()}）`);
...
//   F.1-3: 双窗口扫描（晚到事件：published_at 早于窗口但 first_seen_at 在窗口内）
{
  ...
  const {
    ...
    fetchOfficialEvents,
    computeEventTiming,
  } = await import('../radar_v2_dossier_producer.mjs');
  const { listDossiers, getDossierDetail } = await import('../radar_v2_query_api.mjs');
  ...
  // 模拟 /radar_v2/refresh 的 .then 回调逻辑：扫描完成后按 run 关联
  {
    ...
    // 模拟 /radar_v2/refresh 的 .then 回调：result.runId 非空 + status=complete → 调用 linkObservationsForRun
  ...
  // migration 前创建会报 "no such column" 中止整个 execSchema。
  {
    ...
    // 关键断言：setRadarV2DbForTest 调用 execSchema，旧库不应报 "no such column"
    let initError = null;
    try {
      setRadarV2DbForTest(migDb);
    } catch (e) {
      initError = e;
    }
    ...
    const { reconcilePendingRuns } = await import('../radar_v2_dossier_producer.mjs');
  ...
  // 复现 P1：500 条退避到期的失败 run + 1 条全新 pending run，新 run 必须在本轮被选中。
  {
    ...
    const { getPendingLinkRuns } = await import('../radar_v2_schema.mjs');
...
//       旧 ROUTINE_DISCLOSURE 特殊处理已移除（类型已废弃）。
{
  ...
  const { createDossierFromEvent } = await import('../radar_v2_dossier_producer.mjs');
...
//       不出现在 /opportunities 列表中（INNER JOIN 门槛）。
{
  ...
  const { listOpportunities } = await import('../radar_v2_query_api.mjs');
...
// 前端 "全部状态" 改为并行拉取各 status 合并，依赖后端按 status 精确过滤。
{
  ...
  const { listDossiers } = await import('../radar_v2_query_api.mjs');
...
// 临时回放、缓存重建等 run 即使有 candidate，也不能污染 dossier 的 observation 时间线。
{
  ...
  const { linkObservationsForRun, linkObservationsForMarket, resetLinkWatermarkForTest } = await import('../radar_v2_dossier_producer.mjs');
...
{
  console.log('\n[32] 按股票聚合 API 回归');
  const { listSymbolsAcrossChannels, getDossiersBySymbol } = await import('../radar_v2_query_api.mjs');
...
{
  console.log('\n[33] V2 kline + sparkline API 回归');
  const { getV2Kline, listSparklines } = await import('../radar_v2_query_api.mjs');
...
// 验证：listSymbolsAcrossChannels 按 available_at（非 created_at）判断最新
{
  ...
  const { listSymbolsAcrossChannels } = await import('../radar_v2_query_api.mjs');
