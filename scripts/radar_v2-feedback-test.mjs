...
//
// 运行：node scripts/radar_v2-feedback-test.mjs
...
} from '../radar_v2_feedback.mjs';
import { getActiveWeights, invalidateActiveWeightsCache, DEFAULT_WEIGHTS } from '../radar_v2_scoring.mjs';
...
const tmpDir = mkdtempSync(join(tmpdir(), 'radar-v2-feedback-'));
 ...
 *   - liquidity/reliability 与 forwardReturn 无关（LCG 随机）
 *   - 提高 technical 权重 → composite IC 提高（其他维度是噪声）
 *
 * @param {object} opts
 */
function makeSamples({
  ...
  for (let r = 0; r < runCount; r++) {
    ...
    for (let i = 0; i < perRunCount; i++) {
      // A deterministic cross-section: technical is the only positive
      // predictor, liquidity is deliberately inverse, and reliability is a
      // fixed permutation. This makes the shadow A/B threshold reproducible.
      const technical = 20 + i * 6;
      const liquidity = 80 - i * 6;
      const reliability = 20 + ((i * 3) % perRunCount) * 6;

      let forwardReturn = (rng() - 0.5) * 2 * noiseStd;
      for (const [dim, corr] of Object.entries(dimensionCorrelation)) {
        const dimScore = { technical, liquidity, reliability }[dim];
      ...
      samples.push({
        ...
        entryDate: runAsOf,
        symbol: `SYM${r}_${i}`,
        score: technical * 0.50 + liquidity * 0.25 + reliability * 0.25,
        metrics: { technical, liquidity, reliability },
        forwardReturn,
      });
    }
  }
...
function seedFeedbackData({
  ...
  for (let r = 0; r < runCount; r++) {
    ...
    for (let i = 0; i < perRunCount; i++) {
      // Match the pure-function fixture: improve the technical share against
      // two non-positive dimensions, so a valid shadow must improve IC.
      const technical = 20 + i * 6;
      const liquidity = 80 - i * 6;
      const reliability = 20 + ((i * 3) % perRunCount) * 6;
      const score = technical * 0.50 + liquidity * 0.25 + reliability * 0.25;
      const metrics = { technical, liquidity, reliability };

      // technical 是唯一的确定性强信号；其余两维是噪声。
      // 这让提高 technical 权重的 A/B 改善可重复且显著。
...
console.log('=== 测试 1：suggestWeights 权重调整 ===');
{
  ...
  const dimensionIcs = [
    ...
    { dimension: 'reliability', purged: { count: 10, mean: 0.01 } }, // |IC| < 0.02 → -0.025
  ];
  ...
  assert(adjustments.reliability === -0.025, `reliability 小幅降权 -0.025（实际 ${adjustments.reliability}）`);
...
console.log('=== 测试 2：suggestWeights 样本不足 ===');
{
  const dimensionIcs = [
    ...
    { dimension: 'reliability', purged: { count: 3, mean: 0.1 } },
  ];
  const { newWeights, adjustments } = suggestWeights(DEFAULT_WEIGHTS, dimensionIcs);
  assert(adjustments.technical === 0 && adjustments.liquidity === 0 && adjustments.reliability === 0, '样本不足时所有维度不调整');
...
console.log('=== 测试 4：computeCompositeIc A/B 验证 ===');
{
  ...
  const newWeights = { technical: 0.60, liquidity: 0.20, reliability: 0.20 };
...
// ============================================================
console.log('=== 测试 6：tryGenerateShadow 写入 shadow（确定性） ===');
{
  seedFeedbackData({ market: 'US', runCount: 6, perRunCount: 10 });

  const result = tryGenerateShadow('US', { lookbackDays: 200 });
  ...
  assert(shadow?.sample_count === 60, `sample_count=60（实际 ${shadow?.sample_count}）`);

  console.log(`  improvement: ${result.abTest.improvement}, icOld: ${result.abTest.icOld}, icNew: ${result.abTest.icNew}`);
}
...
console.log('=== 测试 8：getActiveWeights 从 active profile 读取 ===');
{
  ...
  assert(Math.abs(weights.technical - DEFAULT_WEIGHTS.technical) < 0.001, `default technical=0.50（${weights.technical}）`);
  assert(Math.abs(weights.liquidity - DEFAULT_WEIGHTS.liquidity) < 0.001, `default liquidity=0.25（${weights.liquidity}）`);
  assert(Math.abs(weights.reliability - DEFAULT_WEIGHTS.reliability) < 0.001, `default reliability=0.25（${weights.reliability}）`);
...
console.log('=== 测试 10：P0-1 反馈样本隔离正式完整扫描 ===');
{
  ...
  for (let s = 0; s < scenarios.length; s++) {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      const metrics = { technical, event: 50, liquidity: 50, reliability: 50 };
...
console.log('=== 测试 11：P0-2 active profile 拒绝重生成 shadow ===');
{
  ...
  // 生成 shadow
  seedFeedbackData({ market: 'US', runCount: 6, perRunCount: 10 });
  const gen1 = tryGenerateShadow('US', { lookbackDays: 200 });
  ...
  const gen2 = tryGenerateShadow('US', { lookbackDays: 200 });
  ...
  const gen3 = tryGenerateShadow('US', { lookbackDays: 200 });
...
console.log('=== 测试 12：P0-3 权重缓存按 market 隔离 ===');
{
  ...
  // 为 US 和 HK 插入不同的 active profile
  const usWeights = { technical: 0.50, liquidity: 0.25, reliability: 0.25 };
  const hkWeights = { technical: 0.30, liquidity: 0.35, reliability: 0.35 };
  ...
  assert(Math.abs(usResult.technical - 0.5) < 0.001, `US technical=0.5（${usResult.technical}）`);
  ...
  assert(Math.abs(usCached.technical - 0.5) < 0.001, `US 缓存 technical=0.5（${usCached.technical}）`);
  ...
  // 旧的五因子 shadow 与当前三因子 base-score 不兼容，不能被误应用。
  db.prepare(`
    INSERT INTO radar_v2_scoring_profiles (profile_name, market, weights_json, is_active, is_shadow, created_at)
  ...
  assert(legacyApply.ok === false && legacyApply.error.includes('三因子'), '遗留五因子 shadow 被拒绝应用');
