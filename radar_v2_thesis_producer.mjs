...
//
// feature flag: RADAR_V2_THESIS_ENABLED=1（默认关闭，避免 LLM 成本失控）
...
/**
 * thesis 生成是否启用。
 * 必须显式设置 RADAR_V2_THESIS_ENABLED=1，避免 LLM 成本失控。
 */
export function isThesisEnabled() {
  return String(process.env.RADAR_V2_THESIS_ENABLED || '').trim() === '1';
}
...
export async function generateThesisForDossier(dossier, options = {}) {
  ...
  try {
    ...
    console.log(`[radar_v2_thesis] dossier#${dossier.id} LLM 生成失败: ${error.message}`);
 ...
 *
 * 这是 producer 的顶层入口，由 server.mjs 调度（每小时，受 RADAR_V2_THESIS_ENABLED 控制）。
...
export async function produceThesesForDossiers({ limit = MAX_DOSSIERS_PER_BATCH } = {}) {
  ...
  for (const dossier of dossiers) {
    try {
      ...
      console.log(`[radar_v2_thesis] dossier#${dossier.id} 异常: ${e.message}`);
      failed += 1;
    }
  }
