...
//
// 本模块只依赖：
//   - radar_market.mjs    市场适配器 + isAfterClose
//   - radar_scanner.mjs   runScan 扫描入口
//   - market_calendar.mjs    getMarketStatus 交易日历（干净模块，复用）
//   - radar_schema.mjs    scan_jobs 表持久化状态
//
// P0 改造：
...
import { autoAuditProvisionalAssets } from './radar_query_api.mjs';
...
// 配合 radar_rate_limiter.mjs 的 token bucket（60 req/min），全局速率受控。
...
// 任务幂等（守卫式 upsert）。
// 部署实测修正：资产审计不依赖扫描调度开关（RADAR_SCANNER_ENABLED
// 默认关闭，挂在 scheduleRadar 里会导致审计永不执行）——由
...
/**
 * 返回调度器内部状态（供测试和 /radar/status 使用）。
...
export function scheduleRadar({ onRunComplete = null } = {}) {
  ...
  return { check: boundCheck, stop: stopRadar };
}
